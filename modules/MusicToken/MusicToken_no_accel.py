...
class MusicTokenWrapper(torch.nn.Module):
    ...
    def __init__(
        ...
        if args.data_set == 'test':
            ...
            self.embedder.load_state_dict(torch.load(embedder_learned_embeds, map_location=map_device))

            if 'vae' in args and args.vae:
                self.vae.eval()
                vae_learned_embeds = args.learned_vae
                self.vae.load_state_dict(torch.load(vae_learned_embeds, map_location=map_device))

            if 'aud_encoder' in args and args.aud_encoder:
                self.aud_encoder.eval()
                aud_encoder_learned_embeds = args.learned_aud_encoder
                self.aud_encoder.load_state_dict(torch.load(aud_encoder_learned_embeds, map_location=map_device)) 

            if 'unet' in args and args.unet:
                self.unet.eval()
                unet_learned_embeds = args.learned_unet
                self.unet.load_state_dict(torch.load(unet_learned_embeds, map_location=map_device)) 

            if 'lora' in args and args.lora:
                self.lora_layers.eval()
                lora_layers_learned_embeds = args.learned_embeds_lora
                self.lora_layers.load_state_dict(torch.load(lora_layers_learned_embeds, map_location=map_device))
